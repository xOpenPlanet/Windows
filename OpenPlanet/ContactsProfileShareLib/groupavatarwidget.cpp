...
GroupAvatarWidget::GroupAvatarWidget(QWidget *parent)
	: QWidget(parent)
{
	ui.setupUi(this);
...
GroupAvatarWidget::~GroupAvatarWidget()
{

}

//鼠标事件的处理。3
void GroupAvatarWidget::mousePressEvent(QMouseEvent *event)
{
	mouse = event->pos();   //设置移动的原始位置。
	return QWidget::mousePressEvent(event);
}
void GroupAvatarWidget::mouseReleaseEvent(QMouseEvent *event)
{
	mouse.setX(-1);
	return QWidget::mouseReleaseEvent(event);
}
void GroupAvatarWidget::mouseMoveEvent(QMouseEvent *event)
{
	if (mouse.x() < 0)
		return;
	//首先通过做差值，获得鼠标位移的距离。
...
//绘制阴影
void GroupAvatarWidget::paintEvent(QPaintEvent * event)
{
	QPainterPath path;
...
bool GroupAvatarWidget::eventFilter(QObject *obj, QEvent *e)
{	
...
void GroupAvatarWidget::setAvatar(QString headerPath)
{
...
void GroupAvatarWidget::slotCancel()
{
	this->hide();

	setAvatar(groupInfo.groupLoacalHeadImage);
}

void GroupAvatarWidget::slotUploadImage()
{
...
void GroupAvatarWidget::slotConfirmImage()
{
	if (!changed)
	{
		this->hide();
	...
	emit sigUploadImagePath("header.png");
}

void GroupAvatarWidget::upPicFileFinished(bool success)
{
	if (success)
	{
		//删除原头像并用新头像替换。
		QFile::remove(groupInfo.groupLoacalHeadImage);
		QFile::copy("header.png", groupInfo.groupLoacalHeadImage);
...
void GroupAvatarWidget::setGroupInfo(GroupInfo info)
{
	this->groupInfo = info;
	setAvatar(info.groupLoacalHeadImage);
}
